...
inputs = c("cell lines - MYC_BASP1 - mRNA expression (RNA-Seq RPKM).txt",
  "lymphoid_patients - MYC_BASP1 -  (RNA-Seq RPKM).txt",
  "myeloid_patients - MYC_BASP1 -  (RNA-Seq RPKM).txt",
  "bowel_patients - MYC_BASP1 - mRNA Expression, RSEM (Batch normalized from Illumina HiSeq_RNASeqV2).txt")

# function to parse and filter file
parseCsv <- function(file) {
  csv<- read.delim(paste0("input_data/", file))
  csv[which(csv$MYC != "NA" & csv$MYC != 0 & csv$BASP1 != "NA" & csv$BASP1 != 0), ] #filter invalid values
}

allData <- data.frame()
for (filename in inputs) {
  parseCsv(filename)
  data["Origin"] = strsplit(filename, "[A-Z].*")[[1]] #add row with patient cancer origin
  addRow(allData, data) #add data to global list
}
...
# generate a whisker plot from multiple data rows
makePlot <- function(data) {
  ggplot(data, aes(y = Origin, x = log10(MYC/BASP1))) +
    #xlim(0, 7000) +
