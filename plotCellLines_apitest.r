...
genedata = molecularData(api = cbio, 
                         molecularProfileIds = "prad_tcga_pub_rna_seq_v2_mnra", 
                         entrezGeneIds = c("Myc", "Basp1"), 
                         sampleIds = paste0(samples$patientId, '-01'))
...
cellLines<- read.delim("cBioPortalData/cell lines - MYC_BASP1 - mRNA expression (RNA-Seq RPKM).txt")
lymphoidPatients <- read.delim("cBioPortalData/lymphoid_patients - MYC_BASP1 -  (RNA-Seq RPKM).txt")
myeloidPatients <- read.delim("cBioPortalData/myeloid_patients - MYC_BASP1 -  (RNA-Seq RPKM).txt")
